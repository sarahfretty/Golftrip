...
## The teams are uneven, and it favours Gold

Thirteen golfers score, so the sides are **seven (Gold) and six (Aqua)**. The Team Cup counts
each side's best five cards a round, which means Gold discards its two worst cards each round
while Aqua discards only one. More cards to choose from is worth real points.

Simulated over 200,000 rounds, treating every player's Stableford as an equal draw (which is
what the 95% allowance is for), Gold gains roughly **4.6 points a round — about 14 across the
Cup**, now that all three rounds count. It is not decisive: Aqua still wins about 31% of the
...
(Gold nominating six eligible cards a round), or to drop each side's worst card and compare
